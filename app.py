...
# 운영체제에 따른 한글 폰트 설정
# Local Test 할 떄는 windows
# 배포할때는 streamlit 내부 리눅스 가상환경으로 배포가됨
# 배포할때 , 로컬인지 아닌지 확인해라, os 환경이 항상다름.
...
# 티커 파일에서 데이터 로드
@st.cache_data # 크롬 (인터넷브라우저) 메모리사용해서 올려둠.
def load_tickers():
    tickers = {}
    try:
...
try:
    ...
    with tab1:
        ...
        ax.plot(df.index, df['Close'], label='종가')# seaborn 아니구 / matplotlib 문법
    ...
    with tab2:
        ...
        fig, ax = plt.subplots(figsize=(12, 6))
        sns.lineplot(data=df, x=df.index, y='Close', ax=ax) # seaborn문법
        ax.set_title(f"{selected_company} ({ticker}) 주가")
        ax.set_xlabel("날짜")
        ax.set_ylabel("가격 ($)")
        ax.grid(True)
